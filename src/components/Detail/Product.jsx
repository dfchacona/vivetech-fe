  ...
  return (
    <div className="w-screen h-screen bg-slate-300">
      <div className="header flex-col	h-16">
        {loading ? (
            <p className="text-3xl font-bold">{products && products.name}</p>
          ) : (
            <div>
              <p className="text-3xl font-bold">{products && products.name}</p>
              <p className="text-l leading-3">{products && products.description}</p>
            </div>
        )}
        {error && 
      ...
      {products && 
        <div className="w-screen inline-flex flex-col items-center p-5">
          ...
          <table className="table-auto shadow-lg border rounded border-gray-200">
            ...
            <tbody>
              {products.variants.map(({id, name, price}) => {
                return (
                  <tr key={id}>
